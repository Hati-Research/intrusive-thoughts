...
impl<'a> TcpClient<'a> {
    pub fn new(stack: Stack<'a>, rx_buffer: &'a mut [u8], tx_buffer: &'a mut [u8]) -> Self {
        ...
        let handle = cortex_m::interrupt::free(|cs| {
            stack.with(cs, |(sockets, _interface)| sockets.add(socket))
        });
    ...
    pub async fn send(&mut self, buf: &[u8]) -> Result<usize, SendError> {
        poll_fn(|cx| {
            cortex_m::interrupt::free(|cs| {
                self.with(cs, |socket, _context| match socket.send_slice(buf) {
                    Ok(0) => {
                        socket.register_send_waker(cx.waker());
                        Poll::Pending
                    }
    ...
    pub async fn recv(&mut self, buf: &mut [u8]) -> Result<usize, RecvError> {
        poll_fn(|cx| {
            cortex_m::interrupt::free(|cs| {
                self.with(cs, |socket, _context| match socket.recv_slice(buf) {
